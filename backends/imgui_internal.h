...
{
    size_t      Size;           // BufferSize in bytes
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    ImVec2              WorkOffsetMin;          // Work Area: Offset from Pos to top-left corner of Work Area. Generally (0,0) or (0,+main_menu_bar_height). Work Area is Full Area but without menu-bars/status-bars (so WorkArea always fit inside Pos/BufferSize!)
    ImVec2              WorkOffsetMax;          // Work Area: Offset from Pos+BufferSize to bottom-right corner of Work Area. Generally (0,0) or (0,-status_bar_height).
...
struct ImGuiContext
{
    ...
    // Viewports
    ImVector<ImGuiViewportP*> Viewports;                        // Active viewports (BufferSize==1 in 'master' branch). Each viewports hold their copy of ImDrawData.
    ...
    int                             TablesTempDataStacked;      // Temporary table data size (because we leave previous instances undestructed, we generally don't use TablesTempData.BufferSize)
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImVec2                  SizeFull;                           // BufferSize when non collapsed
    ImVec2                  ContentSize;                        // BufferSize of contents/scrollable client area (calculated from the extents reach of the cursor) from previous frame. Does not include window decoration or window padding.
    ImVec2                  ContentSizeIdeal;
    ImVec2                  ContentSizeExplicit;                // BufferSize of contents/scrollable client area explicitly request by the user via SetNextWindowContentSize().
    ImVec2                  WindowPadding;                      // Window padding at the time of Begin().
    float                   WindowRounding;                     // Window rounding at the time of Begin(). May be clamped lower to avoid rendering artifacts with title bar, menu bar etc.
    float                   WindowBorderSize;                   // Window border size at the time of Begin().
    int                     NameBufLen;                         // BufferSize of buffer storing Name. May be larger than strlen(Name)!
    ...
    ImVec2                  ScrollbarSizes;                     // BufferSize taken by each scrollbars on their smaller axis. Pay attention! ScrollbarSizes.x == width of the vertical scrollbar, ScrollbarSizes.y = height of the horizontal scrollbar.
...
struct IMGUI_API ImGuiTable
{
    ...
    ImGuiWindow*                OuterWindow;                // m_ParentBone window for the table
...
struct IMGUI_API ImGuiTableTempData
{
    ...
    int                         HostBackupItemWidthStackSize;//Backup of OuterWindow->DC.ItemWidthStack.BufferSize at the end of BeginTable()
