...
#endif
#define IM_ARRAYSIZE(_ARR)          ((int)(sizeof(_ARR) / sizeof(*(_ARR))))     // BufferSize of a static C-style array. Don't use on pointers!
...
namespace ImGui
{
    ...
    IMGUI_API ImVec2        GetWindowContentRegionMax();                                    // content boundaries max for the full window (roughly (0,0)+BufferSize-Scroll) where BufferSize can be override with SetNextWindowContentSize(), in window coordinates
    ...
    IMGUI_API ImVec2        GetCursorScreenPos();                                           // cursor position in absolute coordinates (useful to work with ImDrawList API). generally top-left == GetMainViewport()->Pos == (0,0) in single viewport mode, and bottom-right == GetMainViewport()->Pos+BufferSize == io.DisplaySize in single-viewport mode.
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_AlwaysVerticalScrollbar= 1 << 14,  // Always show vertical scrollbar (even if ContentSize.y < BufferSize.y)
    ImGuiWindowFlags_AlwaysHorizontalScrollbar=1<< 15,  // Always show horizontal scrollbar (even if ContentSize.x < BufferSize.x)
    ...
    ImGuiWindowFlags_NoDecorationInvisible = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoBackground,
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CallbackResize      = 1 << 18,  // Callback on buffer capacity changes request (beyond 'buf_size' parameter value), allowing the string to grow. Notify when the string wants to be resized (for string types which hold a cache of their BufferSize). You will be provided a new BufSize in the callback and NEED to honor it. (see misc/cpp/imgui_stdlib.h for an example of using this)
...
enum ImGuiKey_
{
    ...
    ImGuiKey_KeysData_SIZE          = ImGuiKey_NamedKey_COUNT,          // BufferSize of KeysData[]: only hold named keys
...
#else
    ImGuiKey_KeysData_SIZE          = ImGuiKey_COUNT,                   // BufferSize of KeysData[]: hold legacy 0..512 keycodes + named keys
...
struct ImGuiIO
{
    ...
    ImVec2      DisplaySize;                    // <unset>          // Main display size, in pixels (generally == GetMainViewport()->BufferSize). May change every frame.
...
struct ImDrawListSplitter
{
    ...
    ImVector<ImDrawChannel>     _Channels;   // Draw channels (not resized down so _Count might be < Channels.BufferSize)
...
// In single viewport mode, top-left is == GetMainViewport()->Pos (generally 0,0), bottom-right is == GetMainViewport()->Pos+BufferSize (generally io.DisplaySize).
...
struct ImDrawList
{
    ...
    unsigned int            _VtxCurrentIdx;     // [Internal] generally == VtxBuffer.BufferSize unless we are past 64K vertices, in which case this gets reset to 0.
...
struct ImDrawData
{
    ...
    int             TotalIdxCount;          // For convenience, sum of all ImDrawList's IdxBuffer.BufferSize
    int             TotalVtxCount;          // For convenience, sum of all ImDrawList's VtxBuffer.BufferSize
    ImDrawList**    CmdLists;               // Array of ImDrawList* to render. The ImDrawList are owned by ImGuiContext and only pointed to from here.
    ImVec2          DisplayPos;             // Top-left position of the viewport to render (== top-left of the orthogonal projection matrix to use) (== GetMainViewport()->Pos for the main viewport, == (0.0) in most single-viewport applications)
    ImVec2          DisplaySize;            // BufferSize of the viewport to render (== GetMainViewport()->BufferSize for the main viewport, == io.DisplaySize in most single-viewport applications)
...
struct ImFontConfig
{
    ...
    float           SizePixels;             //          // BufferSize in pixels for rasterizer (more or less maps to the resulting font height).
...
struct ImGuiViewport
{
    ...
    ImVec2              Size;                   // Main Area: BufferSize of the viewport.
    ImVec2              WorkPos;                // Work Area: Position of the viewport minus task bars, menus bars, status bars (>= Pos)
    ImVec2              WorkSize;               // Work Area: BufferSize of the viewport minus task bars, menu bars, status bars (<= BufferSize)
